...
;(function () {
  ...
  Accordion.prototype.initAccordion = function () {
    ...
    for (let i = 0; i < this.items.length; i++) {
      let button = this.items[i].getElementsByTagName('button')[0],
  ...
  Accordion.prototype.initAccordionEvents = function () {
    ...
    this.element.addEventListener('click', function (event) {
      let trigger = event.target.closest('.js-accordion__trigger')
      //check index to make sure the click didn't happen inside a children accordion
      if (trigger && Util.getIndexInArray(self.items, trigger.parentElement) >= 0)
        self.triggerAccordion(trigger)
    })
  }

  Accordion.prototype.triggerAccordion = function (trigger) {
    let bool = trigger.getAttribute('aria-expanded') === 'true'
  ...
  Accordion.prototype.animateAccordion = function (trigger, bool, deepLink) {
    let self = this
    let item = trigger.closest('.js-accordion__item'),
  ...
  Accordion.prototype.closeSiblings = function (item) {
    ...
    let index = Util.getIndexInArray(this.items, item)
    for (let i = 0; i < this.items.length; i++) {
  ...
  Accordion.prototype.moveContent = function () {
    ...
    let openAccordion = this.element.getElementsByClassName(this.showClass)
    if (openAccordion.length == 0) return
    let boundingRect = openAccordion[0].getBoundingClientRect()
  ...
  Accordion.prototype.initDeepLink = function () {
    ...
    if (!hash || hash == '') return
    let trigger = this.element.querySelector('.js-accordion__trigger[aria-controls="' + hash + '"]')
  ...
  let accordions = document.getElementsByClassName('js-accordion')
